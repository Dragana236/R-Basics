...
# Set up a connection to the MysSQL database
my_db <- src_mysql(dbname = "financial",
                   host = "relational.fit.cvut.cz",
                   port = 3306,
                   user = "guest",
                   password = "relational")
...
# Reference a table within that source: loan
loan <- tbl(my_db, "loan")

# glimpse at loan
glimpse(loan)

# Ordered and grouped summary of loan
loan %>%
  group_by(status) %>%
